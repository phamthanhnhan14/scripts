...
_DATA_DIR=/data/cassandra-test/data
_NODETOOL=/zserver/cassandra_test/bin/nodetool
_CQL=/zserver/cassandra_test/bin/cqlsh
## Custom informations
_USERNAME=cassandra
_PASSWD=cassandra
_HOST=10.30.56.136
...
## List All Keyspaces
$_CQL $_HOST $_PORT -u $_USERNAME -p $_PASSWD -e "DESC KEYSPACES" |perl -pe 's/\e([^\[\]]|\[.*?[a-zA-Z]|\].*?\a)//g' | sed '/^$/d' > Keyspace_name_schema.cql
...
do
$_CQL $_HOST $_PORT -u $_USERNAME -p $_PASSWD -e "DESC KEYSPACE  $VAR_KEYSPACE" > "$_BACKUP_SCHEMA_DIR/$VAR_KEYSPACE/$VAR_KEYSPACE"_schema-"$_DATE_SCHEMA".cql 
done
